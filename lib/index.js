...
module.exports = class Backport extends EE {
  constructor(options) {
    ...
    // targetBranch should probably be the current branch
  ...
  _addListeners() {
    this.on('failed:cherry-pick', (commit, err) => {
      // TODO(evanlucas) add option to ignore failed cherry-picks and return

      read({
        prompt: `(#${commit.pr}): cherry-pick failed. Ask for backport? [y/N]`
      , default: 'n'
      }, (err, result) => {
        if (err) throw err
